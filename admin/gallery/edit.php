<?php 
  ...
  if($_POST){
    ...
    if(isset($_FILES["image"]["name"]) && !empty($_FILES["image"]["name"]))
    {
      ...
      $image                   = 'uploads/gallery/'.$random_file_name;
      if(move_uploaded_file($file_tmp,$physical_dir.$image))
      {
        ...
        $sql       = "UPDATE gallery SET name='$name',image='$image',updated_at='$date' WHERE id=".base64_decode($_GET['id']);

        if ($conn->query($sql) === TRUE) {
          $_SESSION['success'] = 'Record added successfully.';
          echo "<script>location.href = '".$dir."/gallery';</script>";
        } else {
      ...
      $name      = $_POST['name'];
      $date      = date('Y-m-d h:i:s');
      $sql       = "UPDATE gallery SET name='$name',updated_at='$date' WHERE id=".base64_decode($_GET['id']);
  ...
  $sql       = "SELECT * FROM gallery WHERE id=".base64_decode($_GET['id'])." limit 1";
